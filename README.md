...
Pitboss is a dual-purpose blackjack system that aims to serve both as a realistic game implementation and a simulation framework. The project takes an unconventional approach by modeling blackjack as multiplexed 1v1 "bouts" between dealer and contestants, emphasizing correctness through careful domain modeling.

A "bout" represents the fundamental interaction in blackjack: a single contestant competing against the dealer. While blackjack appears to be one dealer versus multiple contestants, it's actually several independent competitions happening in parallel. This bout-centric modeling simplifies simulation logic, enables more efficient parallelization, and makes it easier to analyze individual hand outcomes without the complexity of table-level interactions.
