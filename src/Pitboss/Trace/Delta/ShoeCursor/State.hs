...
module Pitboss.Trace.Delta.ShoeCursor.State where
...
instance Incremental ShoeCursorStateDelta where
  ...
  applyDelta delta state = case delta of
    Advance n -> state {_offset = _offset state + n}
    Rewind n -> state {_offset = _offset state - n}
    ReplaceOffset _ new -> state {_offset = new}
