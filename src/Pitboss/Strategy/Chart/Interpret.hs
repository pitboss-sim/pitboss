...
-- Direct lookup using typed hands - no conversion needed!
...
findMatchingEntry chart hand@(SomeHand h) =
    case witness h of
        ...
        BustWitness ->
            -- For busted hands, we typically don't need strategy decisions
            -- but if we do, treat as hard hand with score 0
