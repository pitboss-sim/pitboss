...
data EntityTickCache = EntityTickCache
...
makeLenses ''EntityTickCache

data TickCacheContext = TickCacheContext
    { _ctxTickCache :: EntityTickCache
    , _ctxTick :: Tick
    }
...
mkTickCache :: Tick -> EntityTickCache
mkTickCache tick =
    EntityTickCache
...
populateTickCache playerReg handReg spotReg dealerReg dealerHandReg dealerRoundReg offeringReg tableReg tableShoeReg tick =
    let baseTickCache = mkTickCache tick
        ...
        populateFromRegistry ::
            ...
            (EntityTickCache -> IHM.InsOrdHashMap Word64 (EntityState k)) ->
            (EntityTickCache -> IHM.InsOrdHashMap Word64 (EntityState k) -> EntityTickCache) ->
            EntityTickCache ->
            EntityTickCache
...
withTickCache :: EntityTickCache -> Reader TickCacheContext a -> a
