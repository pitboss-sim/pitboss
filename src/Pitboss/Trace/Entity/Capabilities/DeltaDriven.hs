...
import Pitboss.FSM.PlayerHandFSM.Types hiding (Blackjack)
...
instance DeltaDriven PlayerHandState PlayerHandDelta where
  ...
  describeDelta d _ = case d of
    ...
    ReplacePlayerHandIndex from to -> "Changed hand index: " ++ show from ++ " → " ++ show to
...
--
-- instance Validates PlayerHandState PlayerHandDelta PlayerHandDeltaViolation where
...
data PlayerHandDeltaViolation
  = IllegalBetChange
      { attempted :: PlayerHandDelta,
        reason :: String,
        phase :: HandPhase
      }
  | IllegalMutation
  ...
  | IllegalPhase
      { attemptedGroup :: [PlayerHandDelta],
        reason :: String,
        phase :: HandPhase
      }
  deriving (Eq, Show)

legalPlayerHandDeltas :: SomePlayerHandFSM -> [PlayerHandDelta] -> Either PlayerHandDeltaViolation [PlayerHandDelta]
legalPlayerHandDeltas (SomePlayerHandFSM fsm) deltas =
  case fsm of
    AbandonedFSM _ ->
...
validateInPhase :: HandPhase -> PlayerHandDelta -> Either PlayerHandDeltaViolation PlayerHandDelta
...
--     OneCardDrawFSM reason ->
--       mapM (validateInPhase (OneCardDraw reason)) deltas
-- legalPlayerHandDeltas ::
--   SomePlayerHandFSM ->
--   StateSnapshot PlayerHandState PlayerHandDelta ->
--   (Tick -> Tick) ->
--   [PlayerHandDelta] ->
--   Either PlayerHandDeltaViolation (StateSnapshot PlayerHandState PlayerHandDelta)
-- legalPlayerHandDeltas (SomePlayerHandFSM _fsm) snapshot bump =
--   foldl step (Right snapshot)
--   where
--     step acc delta = acc >>= applyIfValid bump delta
-- validateInFSM :: HandFSM p h d s -> PlayerHandDelta -> Either PlayerHandDeltaViolation PlayerHandDelta
