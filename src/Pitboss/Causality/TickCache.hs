...
data TickCache = TickCache
    ...
    , _cacheContestant :: IHM.InsOrdHashMap ContestantId (EntityState 'Contestant)
    , _cacheDealer :: IHM.InsOrdHashMap DealerId (EntityState 'Dealer)
    , _cacheRound :: IHM.InsOrdHashMap RoundId (EntityState 'Round)
    , _cacheHand :: IHM.InsOrdHashMap HandId (EntityState 'Hand)
...
derefHelper ::
    (MonadReader TickCacheContext m, Eq id, Hashable id) =>
...
instance (MonadReader TickCacheContext m) => Deref ContestantId m where
    type DerefTarget ContestantId = EntityState 'Contestant
    deref = derefHelper cacheContestant
...
instance (MonadReader TickCacheContext m) => Deref HandId m where
    type DerefTarget HandId = EntityState 'Hand
    deref = derefHelper cacheHand
...
mkTickCache tick =
    TickCache
        ...
        , _cacheContestant = IHM.empty
        , _cacheDealer = IHM.empty
        , _cacheHand = IHM.empty
...
populateTickCache ::
    ...
    Registry 'Contestant (SomeDelta 'Contestant) ->
    Registry 'Dealer (SomeDelta 'Dealer) ->
    Registry 'Hand (SomeDelta 'Hand) ->
    ...
    Tick ->
    TickCache
populateTickCache boutReg contestantReg dealerReg handReg playerReg roundReg shoeReg tableReg tick =
    let baseTickCache = mkTickCache tick
        populateFromRegistry ::
            (IncrementalWithWitness k, Eq (EntityIdFor k), Hashable (EntityIdFor k)) =>
     ...
     in baseTickCache
            ...
            & populateFromRegistry contestantReg _cacheContestant (\c m -> c{_cacheContestant = m})
            & populateFromRegistry dealerReg _cacheDealer (\c m -> c{_cacheDealer = m})
            & populateFromRegistry handReg _cacheHand (\c m -> c{_cacheHand = m})
