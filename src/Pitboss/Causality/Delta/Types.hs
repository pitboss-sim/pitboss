...
data instance Delta 'Bout ('PartialUpdate 'Rels)
    = DBoutSetPlayerHand HandId HandId
    | DBoutSetDealerHand HandId HandId
    | DBoutSetRound RoundId RoundId
    | DBoutSetShoe ShoeId ShoeId
    | DBoutSetTable TableId TableId
    deriving (Eq, Show, Generic)

data instance Delta 'Contestant ('PartialUpdate 'Attrs)
    = DContestantSetActiveHandIx HandIx HandIx
    deriving (Eq, Show, Generic)

data instance Delta 'Contestant ('PartialUpdate 'Modes)
    = DContestantSetBoutFSM (Occupancy SomeContestantBoutFSM) (Occupancy SomeContestantBoutFSM)
    | DContestantSetRoundFSM SomeContestantRoundFSM SomeContestantRoundFSM
    deriving (Eq, Show, Generic)

data instance Delta 'Contestant ('PartialUpdate 'Rels)
    = DContestantSetBoutEntry HandIx (Occupancy BoutId) (Occupancy BoutId)
    | DContestantInsertBoutEntry HandIx (Occupancy BoutId)
    | DContestantSetPlayer PlayerId PlayerId
    | DContestantSetRound RoundId RoundId
    | DContestantSetShoe ShoeId ShoeId
    deriving (Eq, Show, Generic)
...
data instance Delta 'Dealer ('PartialUpdate 'Rels)
    = DDealerSetActiveHand (Occupancy HandId) (Occupancy HandId)
    | DDealerSetActiveRound (Occupancy RoundId) (Occupancy RoundId)
    | DDealerSetActiveTable (Occupancy TableId) (Occupancy TableId)
    deriving (Eq, Show, Generic)

data instance Delta 'Hand ('PartialUpdate 'Attrs)
    = DHandSetHand SomeHand SomeHand
    deriving (Eq, Show, Generic)

data instance Delta 'Hand ('PartialUpdate 'Modes)
    = DHandSetHandFSM SomeHandFSM SomeHandFSM
    deriving (Eq, Show, Generic)

data instance Delta 'Hand ('PartialUpdate 'Rels)
    = DHandSetOwner HandOwner HandOwner
    | DHandSetBout BoutId BoutId
    | DHandSetShoe ShoeId ShoeId
    deriving (Eq, Show, Generic)
...
data instance Delta 'Round ('PartialUpdate 'Rels)
    = DRoundSetDealerHand (Occupancy HandId) (Occupancy HandId)
    | DRoundSetContestantEntry TableSpotIx (Occupancy ContestantId) (Occupancy ContestantId)
    | DRoundInsertContestantEntry TableSpotIx (Occupancy ContestantId)
...
data instance Delta 'Shoe ('PartialUpdate 'Attrs)
    ...
    | DShoeInsertCardFate CardIx CardState
    deriving (Eq, Show, Generic)
...
data instance Delta 'Table ('PartialUpdate 'Attrs)
    ...
    | DTableInsertSeatEntry TableSpotIx (Occupancy TableSeat)
    deriving (Eq, Show, Generic)
