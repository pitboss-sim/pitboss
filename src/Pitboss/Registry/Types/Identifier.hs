...
module Pitboss.Registry.Types.Identifier where
...
newtype SpotHandId = SpotHandId Uid deriving stock (Eq, Ord)

instance Show SpotHandId where show (SpotHandId (Uid u)) = "H" ++ u

mkSpotHandId :: Uid -> SpotHandId
mkSpotHandId = SpotHandId
...
instance Hashable TableId where
  hashWithSalt salt (TableId u) = hashWithSalt salt u
...
instance Hashable SpotHandId where
  hashWithSalt salt (SpotHandId u) = hashWithSalt salt u
