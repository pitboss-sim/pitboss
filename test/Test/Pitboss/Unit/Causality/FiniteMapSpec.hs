...
type HandIdFiniteMap = FiniteMap HandIx (Occupancy HandId)
...
emptyHandIdMap :: HandIdFiniteMap
emptyHandIdMap = emptyFiniteMap Absent
...
spec = describe "FiniteMap" $ do
    describe "creation and access" $ do
        ...
        it "works with entity relationships" $ do
            let fm' = insertFiniteMap Hand2 (Present (HandId largeTestValue)) emptyHandIdMap
            fm' `shouldLookupTo` Hand2 $ Present (HandId largeTestValue)

        it "singleton works correctly" $ do
            let fm = singletonFiniteMap Hand3 (Present (HandId smallTestValue)) Absent
            fm `shouldLookupTo` Hand3 $ Present (HandId smallTestValue)
