...
reconstructAt timeline tick = do
  ...
  where
    applyDeltaToEntity entity delta = case delta of
        AttrsDelta _ d -> Right $ applyWithWitness AttrsWitness d entity
        ModesDelta _ d -> Right $ applyWithWitness ModesWitness d entity
        RelsDelta _ d -> Right $ applyWithWitness RelsWitness d entity
