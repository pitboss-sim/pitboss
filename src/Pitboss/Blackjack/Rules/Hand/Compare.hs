...
compareHandsForBout contestantHand dealerHand = case compareHands contestantHand dealerHand of
    GT -> determineWinReason contestantHand dealerHand ContestantWin
    LT -> determineWinReason contestantHand dealerHand DealerWin
    EQ -> pushOutcome

determineWinReason :: SomeHand -> SomeHand -> BoutOutcome -> DetailedOutcome
determineWinReason contestantHand dealerHand winner =
    let contestantWitness = handWitness contestantHand
        dealerWitness = handWitness dealerHand
     in case (valueType contestantWitness, valueType dealerWitness, winner) of
            (_, BustWitness, ContestantWin) -> contestantWinsDealerBust
            (BustWitness, _, DealerWin) -> dealerWinsContestantBust
            (BlackjackWitness, _, ContestantWin) -> contestantWinsBlackjack
            (_, BlackjackWitness, DealerWin) -> dealerWinsBlackjack
            (_, _, ContestantWin) -> contestantWinsHigher
            (_, _, DealerWin) -> dealerWinsHigher
            _ -> pushOutcome
...
compareByOutcome out1 out2 = case (outcome out1, outcome out2) of
    (ContestantWin, ContestantWin) -> EQ
    (DealerWin, DealerWin) -> EQ
    (Push, Push) -> EQ
    (ContestantWin, _) -> GT
    (Push, DealerWin) -> GT
    (DealerWin, _) -> LT
