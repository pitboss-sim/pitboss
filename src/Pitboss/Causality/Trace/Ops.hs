    ...
    (BirthOp w1 eid1 state1) == (BirthOp w2 eid2 state2) =
        case (w1, w2) of
            ...
            (HandWitness, HandWitness) ->
                eid1 == eid2 && state1 == state2
            (ContestantWitness, ContestantWitness) ->
                eid1 == eid2 && state1 == state2
...
instance ToJSON TraceOp where
    toJSON (BirthOp witness' eid state) = case witness' of
        ...
        ContestantWitness -> object ["type" .= String "BirthOp", "kind" .= String "Contestant", "entityId" .= eid, "state" .= state]
        ...
        HandWitness -> object ["type" .= String "BirthOp", "kind" .= String "Hand", "entityId" .= eid, "state" .= state]
    ...
    toJSON (DeathOp witness' eid reason) = case witness' of
        ...
        HandWitness -> object ["type" .= String "DeathOp", "kind" .= String "Hand", "entityId" .= eid, "reason" .= reason]
        _ -> object []
        ...
        case opType of
            "BirthOp" -> do
                ...
                case kind of
                    ...
                    "Contestant" -> BirthOp ContestantWitness <$> obj .: "entityId" <*> obj .: "state"
                    "Dealer" -> BirthOp DealerWitness <$> obj .: "entityId" <*> obj .: "state"
                    "Round" -> BirthOp RoundWitness <$> obj .: "entityId" <*> obj .: "state"
                    "Hand" -> BirthOp HandWitness <$> obj .: "entityId" <*> obj .: "state"
            ...
            "MutationOp" -> do
                ...
                case kind of
                    ...
                    "Contestant" -> MutationOp ContestantWitness <$> obj .: "entityId" <*> obj .: "delta"
                    "Dealer" -> MutationOp DealerWitness <$> obj .: "entityId" <*> obj .: "delta"
                    "Round" -> MutationOp RoundWitness <$> obj .: "entityId" <*> obj .: "delta"
                    "Hand" -> MutationOp HandWitness <$> obj .: "entityId" <*> obj .: "delta"
            ...
            "DeathOp" -> do
                ...
                case kind of
                    ...
                    "Hand" -> DeathOp HandWitness <$> obj .: "entityId" <*> obj .: "reason"
...
applyTraceOp (BirthOp witness' eid state) tick trace =
    case witness' of
        ...
        ContestantWitness -> applyBirth @'Contestant eid state tick trace
        PlayerWitness -> applyBirth @'Player eid state tick trace
        DealerWitness -> applyBirth @'Dealer eid state tick trace
        HandWitness -> applyBirth @'Hand eid state tick trace
...
applyTraceOp (MutationOp witness' eid someDelta) tick trace =
    case witness' of
        ...
        ContestantWitness -> applyMutation @'Contestant eid someDelta tick trace
        PlayerWitness -> applyMutation @'Player eid someDelta tick trace
        DealerWitness -> applyMutation @'Dealer eid someDelta tick trace
        HandWitness -> applyMutation @'Hand eid someDelta tick trace
...
createBirth :: (Witnessable k, EntityIdClass (EntityIdFor k)) => EntityIdFor k -> EntityState k -> TraceOp
createBirth eid state = BirthOp (witness state) eid state

createMutation :: (Witnessable k, EntityIdClass (EntityIdFor k)) => EntityIdFor k -> EntityState k -> SomeDelta k -> TraceOp
createMutation eid state = MutationOp (witness state) eid

createDeath :: (Witnessable k, EntityIdClass (EntityIdFor k)) => EntityIdFor k -> EntityState k -> DeathReason k -> TraceOp
